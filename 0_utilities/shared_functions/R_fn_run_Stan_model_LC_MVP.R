...
R_fn_run_Stan_model_LC_MVP <- function(    computer, 
                                           run_type, 
                                           Stan_model_obj,
                                           Stan_data_list_given_current_N,
                                           global_list, 
       ...
       km1_choose_2 <- choose(n_tests - 1, 2)
       known_num <- 0 
       ...
       inits_all_chains <- rep(list(stan_inits_list), n_chains)
       ##
       # length(inits_all_chains)
       # 
       # 
       # # ##
       # Stan_data_list_given_current_N$n_class
       # Stan_data_list_given_current_N$n_pops
       # Stan_data_list_given_current_N$pop
       # Stan_data_list_given_current_N$n_covariates_max_nd
       # Stan_data_list_given_current_N$n_covariates_max_d
       # Stan_data_list_given_current_N$n_covariates_max
       # str(Stan_data_list_given_current_N$X_nd)
       # str(Stan_data_list_given_current_N$X_d)
       # ##
       # Stan_data_list_given_current_N$n_covs_per_outcome
       # ##
       # Stan_data_list_given_current_N$corr_force_positive
       # Stan_data_list_given_current_N$known_num
       # ##
       # Stan_data_list_given_current_N$overflow_threshold
       # Stan_data_list_given_current_N$underflow_threshold
       # ##
       # Stan_data_list_given_current_N$prior_only
       # Stan_data_list_given_current_N$prior_beta_mean
       # Stan_data_list_given_current_N$prior_beta_sd
       # Stan_data_list_given_current_N$prior_LKJ
       # Stan_data_list_given_current_N$prior_p_alpha
       # Stan_data_list_given_current_N$prior_p_beta
       # ##
       # Stan_data_list_given_current_N$Phi_type
       # Stan_data_list_given_current_N$handle_numerical_issues
       # Stan_data_list_given_current_N$fully_vectorised
       # # ##
       # # 
       # # 
       # str(inits_all_chains[[1]]$u_raw)
       # str(inits_all_chains[[1]]$p_raw)
       # str(inits_all_chains[[1]]$beta_vec)
       # str(inits_all_chains[[1]]$off_raw)
       # str(inits_all_chains[[1]]$col_one_raw)
       ##
       # inits_all_chains[[1]]$u_raw
       
       #### ---- Run Stan model:
       model <- Stan_model_obj$sample(    data = Stan_data_list_given_current_N,
                                          seed = MCMC_seed,
                                          chains = n_chains,
       ...
       gc(reset = TRUE)
        ...
        total_time_seconds <- time_stan_total_inc_csv_inner_numeric #  cmdstanr_model_out_timers$total
        ...
        stan_total_gradient_evals <-      (mean_L_post_burnin*n_chains*iter_sampling + mean_L*n_chains*iter_warmup)   #  n_chains * iter_sampling * stan_n_leapfrogs ; stan_total_gradient_evals
        ...
        #### ------ SAVE KEY OUTPUTS:
        stan_file_name_string <- paste0(    "run_type", run_type, "_",
        ...
        ## 1. ------  save FULL cmdstanr output (WARNING: file may be massive!)
        ...
        #### ---- Model run time ("inner" timer) - using tictoc:
        {
          ...
          time_stan_total_inc_csv_outer_numeric <- as.numeric(extract_numeric_string)
        }
        ...
        file_path <- file.path(save_output_directory, file_name)
        ## save RDS file:
        saveRDS(object = Stan_key_info_list, file = file_path)
