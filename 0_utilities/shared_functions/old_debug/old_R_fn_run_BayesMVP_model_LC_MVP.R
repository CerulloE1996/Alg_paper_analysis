     ...
     for (df_index in start_index:length(N_vec[!is.na(N_vec)]))  {
       ...
       for (k in (1:length(n_chains_burnin_vec))) {
                         ...
                         for (n_burn_index in (1:length(n_burnin_vec))) {
                               ...
                               for (n_iter_index in (1:length(n_iter_vec))) {
                                     ...
                                     for (iii in 1:n_runs) {
                                           ...
                                           total_gradient_evals <-      (mean_L_post_burnin*n_chains_sampling*iter_sampling + mean_L_burnin*n_chains_burnin*iter_warmup)    
                                           total_gradient_evals_pb <-   (mean_L_post_burnin*n_chains_sampling*iter_sampling)
                                           ##
                                           min_ess_per_grad_eval <- Min_ESS / total_gradient_evals ; min_ess_per_grad_eval
                                           min_ess_per_grad_eval_pb <- Min_ESS / total_gradient_evals_pb ; min_ess_per_grad_eval_pb
                                           ##
                                           ...
                                           if (save_full_outputs_of_single_runs == TRUE) {
                                             ## File name and path:
            ...
            BayesMVP_efficiency_info_list <- list(  paste("total_divs = "), total_divs,
                                                ...
                                                ##
                                                paste("stan_total_gradient_evals = "), total_gradient_evals,
                                                paste("stan_total_gradient_evals_pb = "), total_gradient_evals_pb,
                                                ##
                                                paste("stan_min_ess_per_grad_eval = "), min_ess_per_grad_eval,
                                                paste("stan_min_ess_per_grad_eval_pb = "), min_ess_per_grad_eval_pb,
                                                ##
