    ...
    
    {
        ...
        tibble_all_runs_skeleton$device
        ## Add outputs to list:
        ...
        plots_and_table_outs <- R_fn_ps_opt_N_chunks_plots(tibble_all_runs = tibble_all_runs,
                                                           ...
                                                           global_list = global_list,
                                                           output_path = pilot_study_opt_N_chunks_list$output_path,
                                                           plot_1_overall_scale_factor = 1,
                                                           plot_1_width_scale_factor = 16,
                                                           plot_1_height_scale_factor = 18, 
    ...
    #### -------  Pilot study #3 - "Finding an appropriate minimum target ESS for each N"    -------------------------------------------------------------------------
    source(file.path(getwd(), "0_utilities/shared_functions/R_fn_compile_BayesMVP_model.R"))
    # {
    #   ##
    #   ## Compile Stan model using custom optimised C++ flags (to make "fairer" comparison with BayesMVP):
    ...
    #### -------   Run Pilot study #3  (using BayesMVP - "standard" SNAPER-HMC) Pilot Study (Note: All pilot studies were run on my local HPC) -----------------------
    ##
    {
          ## Set Stan pilot study parameters:
          pilot_study_target_ESS_list <- list()
          ##
          ## BayesMVP_pilot_study_list$Stan_data_list <- Stan_data_list
          pilot_study_target_ESS_list$model_args_list <- model_args_list
          ##
          path_ps3 <- file.path(getwd(), 
                                "1_appendix_pilot_studies", 
                                "ps_3_determining_target_ESS_for_benchmarks")
          pilot_study_target_ESS_list$path <- path_ps3
          pilot_study_target_ESS_list$output_path <- file.path(path_ps3, "outputs")
          ##
          pilot_study_target_ESS_list$N_vec <- c(500, 1000, 2500, 5000, 12500, 25000)
          pilot_study_target_ESS_list$n_runs_per_N <- 10
          pilot_study_target_ESS_list$n_runs <- 10
          pilot_study_target_ESS_list$start_index <- 5
          ## Set # chains and # superchains 
          ## NOTE: We ran all pilot studies on a local HPC with 96 cores, and we used 32 chains/threads for each models run. 
          ## Hence, we set n_chains to min(32, parallel::detectcores()) to accommodate "normal" PC's with less cores. 
          pilot_study_target_ESS_list$n_chains_burnin_vec <- c(min(8, parallel::detectCores()))
          pilot_study_target_ESS_list$n_chains_sampling_vec <- c(min(64, parallel::detectCores()))
          ##
          # n_superchains: E.g. if we have 32 cores then we have 4 superchsins so each superchain has 8 chains (since 4x8 = 32)
          pilot_study_target_ESS_list$n_superchains <- round(pilot_study_target_ESS_list$n_chains_sampling / 8) 
          ##
          pilot_study_target_ESS_list$n_burnin_vec <- c(500)
          pilot_study_target_ESS_list$n_iter_vec <-   c(1000)
          ##
          pilot_study_target_ESS_list$adapt_delta <- 0.80
          pilot_study_target_ESS_list$learning_rate <- 0.05
          ##
          pilot_study_target_ESS_list$partitioned_HMC <- FALSE
          pilot_study_target_ESS_list$diffusion_HMC <-   FALSE
          pilot_study_target_ESS_list$sample_nuisance <- FALSE  
          ##
          pilot_study_target_ESS_list$metric_shape_main <- "dense"
          pilot_study_target_ESS_list$metric_type_main <- "Hessian"
          ##
          pilot_study_target_ESS_list$clip_iter <- 25
          ##
          pilot_study_target_ESS_list$interval_width_main <- 50
          ##
          pilot_study_target_ESS_list$tau_mult <- 1.60
          ##
          pilot_study_target_ESS_list$ratio_M_us <- 0.25
          pilot_study_target_ESS_list$ratio_M_main <- 0.25
          ##
          pilot_study_target_ESS_list$force_autodiff <-   FALSE
          pilot_study_target_ESS_list$force_PartialLog <- FALSE
          pilot_study_target_ESS_list$multi_attempts <-   TRUE
          ##
          pilot_study_target_ESS_list$pilot_study_parallel_scaling_comp_list <- pilot_study_parallel_scaling_comp_list
          ##
          pilot_study_target_ESS_list$N_sample_sizes_vec_for_BayesMVP <- global_list$N_sample_sizes_vec[1:6] 
    }
    
 

    # ## -------- Run the pilot study (ps3): --------------------------------------------------------------------------------------
    source(file.path(getwd(), "0_utilities/shared_functions/R_fn_BayesMVP_file_name_str.R"))
    source(file.path(getwd(), "0_utilities/shared_functions/R_fn_run_BayesMVP_model_LC_MVP.R"))
    
    R_fn_run_BayesMVP_model_LC_MVP(computer = computer,
                                   run_type = "ps3",
                                   ##
                                   BayesMVP_model_obj = BayesMVP_LC_MVP_model_using_manual_grad_obj,
                                   ...
                                   SIMD_vect_type = BayesMVP:::detect_vectorization_support(),
                                   ##
    ...
    ## --------- Load the data saved from running models  ------------------------------------------------------------------------
    source(file.path(path_ps3, "functions/R_fn_ps3_make_tibbles.R"))
    ## First get file name string:
    BayesMVP_BASELINE_file_name_string <- R_fn_ps3_target_ESS_make_tibbles(  run_type = "ps3",
                                                                             computer = "Local_HPC",
                                                                             seed = seed,
    ...
    ## First load the SUMMARY data:
    {
      ...
      file_name <- paste0("BayesMVP_EFF_info", "_", 
                          "n_runs", pilot_study_target_ESS_list$n_runs, "_", 
                          BayesMVP_BASELINE_file_name_string)
      file_path <- file.path(pilot_study_target_ESS_list$output_path, file_name)
      ## Load RDS file:
      df_alg_efficiency_all_runs_all_N <- readRDS(file_path)
    }
    
    R_fn_ps_target_ESS_compute
    
    
    
    
    # for (N in pilot_study_target_ESS_list$N_sample_sizes_vec_for_BayesMVP) {
    #   
    #   ## Stan_data_list_given_current_N <- Stan_data_list[[df_index]]
    #   
    #   for (n_burnin in pilot_study_target_ESS_list$n_burnin_vec) {
    #     
    #     for (i in 1:pilot_study_target_ESS_list$n_runs) {
    #       
    #       # ## For testing: 
    #       # {
    #       #     global_list = global_list
