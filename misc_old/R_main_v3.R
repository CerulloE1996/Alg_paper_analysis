    ...
    ##
    {
      ...
      ##
      path_ps3 <- file.path(getwd(), "1_appendix_pilot_studies", "ps_3_determining_target_ESS_for_benchmarks")
      ...
      pilot_study_target_ESS_list$start_index <- 1
      ## Set # chains and # superchains 
      ## NOTE: We ran all pilot studies on a local HPC with 96 cores, and we used 32 chains/threads for each models run. 
      ## Hence, we set n_chains to min(32, parallel::detectcores()) to accommodate "normal" PC's with less cores. 
      ...
      pilot_study_target_ESS_list$n_iter_vec <-   c(2500)
      ##
      ...
      pilot_study_target_ESS_list$sample_nuisance <- TRUE  
      ##
    ...
    source(file.path(path_ps3, "functions", "file.R"))
    
    ## For Stan, due to time constraints we "only" did N = {500, 1000, 2500, 5000} (i.e., excluding the 2 very large datasets):
    pilot_study_target_ESS_list$N_sample_sizes_vec_for_BayesMVP <- global_list$N_sample_sizes_vec[1:6] 
    # ## Fixed:
    # n_iter <-    pilot_study_target_ESS_list$n_iter
    # n_chains_burnin <-    pilot_study_target_ESS_list$n_chains_burnin
    # n_chains_sampling <-    pilot_study_target_ESS_list$n_chains_sampling
    ...
    ## 
    # 
    # str(pilot_study_target_ESS_list)
    # 
    # pilot_study_target_ESS_list$model_args_list
